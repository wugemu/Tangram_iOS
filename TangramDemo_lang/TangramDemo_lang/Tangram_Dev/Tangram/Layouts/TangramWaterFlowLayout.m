...
-(void)calculateLayout
{
    ...
    for (NSUInteger i = 0; i < self.itemModels.count; i++) {
        ...
        if(i%self.numberOfColumns<self.bottomRects.count){
            self.minRect = [[self.bottomRects allValues][i%self.numberOfColumns] CGRectValue];
        }
        ...
        // 更新最下边一行的记录字典
        [self.bottomRects tm_safeSetObject:[NSValue valueWithCGRect:cellFrame] forKey:[NSString stringWithFormat:@"%d",(int)(i%self.numberOfColumns)]];
        // 先随便给一个值，然后去最后一行找
