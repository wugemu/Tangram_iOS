...
-(void)setLoadNumber:(NSUInteger)preLoadNumber{
    self.preLoadNumber=preLoadNumber;
}
-(void)setEnableAutoLoadMore:(BOOL)mEnableAutoLoadMore{
    self.mEnableAutoLoadMore=mEnableAutoLoadMore;
    self.page=1;
    self.isLoadMore=NO;
}
...
- (UIView *)scrollView:(TMMuiLazyScrollView *)scrollView itemByMuiID:(NSString *)muiID
{
    ...
    if (self.clDataSource
                ...
                && [layout respondsToSelector:@selector(addFooterView:)])
        {
            ...
            else if(!self.isLoadMore){//防止部分UI重复添加
        ...
        //抛出可异步加载的事件 分页加载
        if (_mEnableAutoLoadMore&&([layout isKindOfClass:[TangramFlowLayout class]]||[layout isKindOfClass:[TangramWaterFlowLayout class]])&&(layout.loadType == TangramLayoutLoadTypeLoadOnce || layout.loadType == TangramLayoutLoadTypeByPage) && layout.loadAPI.length > 0 && indexInLayout == self.muiIDModelIndex.count-self.preLoadNumber) {
            TangramEvent *loadEvent = [[TangramEvent alloc]initWithTopic:@"requestItems" withTangramView:self posterIdentifier:@"requestItems" andPoster:self];
            [loadEvent setParam:layout.loadAPI forKey:@"loadAPI"];
            [loadEvent setParam:[NSString stringWithFormat:@"%ld",self.page] forKey:@"page"];
            [loadEvent setParam:layout forKey:@"Card"];
            [loadEvent setParam:[NSNumber numberWithInteger:layout.loadType] forKey:@"loadType"];
            if (layout.loadParams.count > 0) {
                [loadEvent setParam:layout.loadParams forKey:@"loadParams"];
            }
            [layout.tangramBus postEvent:loadEvent];
            self.page++;
        }
    }
...
- (void)reloadData
{
    ...
    if (self.clDataSource
        ...
        [self.muiIDModelIndex removeAllObjects];
